...
| URL công khai | TODO: URL HTTPS của CloudFront |
...
|---|---|---|
| 1. Giao diện người dùng | CloudFront + S3 static frontend | TODO: ảnh chụp màn hình CloudFront distribution |
...
| 3. Tính năng AI / ML | Amazon Bedrock Claude 3.5 Haiku + Bedrock Knowledge Base / gọi trực tiếp InvokeModel làm dự phòng | TODO: quyền truy cập mô hình Bedrock + kết quả UI |
| 4. Lưu trữ dữ liệu | DynamoDB (trạng thái người dùng/tài liệu/truy vấn); S3 JSON (cho flashcard/bài kiểm tra) | TODO: ảnh chụp màn hình item DynamoDB + đối tượng S3 |
...
| Lưu trữ Object | S3 | PDF, nội dung trích xuất, flashcard, và JSON bài kiểm tra đều là dữ liệu blob/tài liệu. |
| Mô hình AI | Claude 3.5 Haiku | Mô hình rẻ nhất nhưng đủ khả năng để tạo hỏi đáp/flashcard/bài kiểm tra học tập trong thời gian hackathon. |
...
- Cảnh báo ngân sách được cấu hình qua SAM dưới tên `studybot-monthly-budget-G15` với thông báo SNS ở mức 30%, 50%, và 80%.
...
### Quyết định 1: Sử dụng Claude 3.5 Haiku cho việc tạo tài liệu học tập trong suốt hackathon

**Các lựa chọn thay thế đã xem xét**

- Claude 3.5 Sonnet: bị loại cho việc sử dụng mặc định vì nó đắt gấp khoảng 3 lần so với Haiku trên mỗi token đầu vào/đầu ra và khối lượng công việc demo không yêu cầu suy luận sâu trên các tài liệu dài và phức tạp.
...
- TODO: Chạy 5 prompt đại diện và ghi lại độ trễ trung bình: `___ ms`.
- TODO: Ghi lại tỷ lệ câu trả lời chấp nhận được trên 5 câu hỏi học tập: `___/5`.
- Tham khảo giá từ ước tính chi phí W7: Haiku `$1.00 / 1 triệu token đầu vào` và `$5.00 / 1 triệu token đầu ra`; Sonnet cao hơn khoảng 3 lần.
...
- Haiku có thể tạo ra các suy luận kém trau chuốt hơn Sonnet trên tài liệu phức tạp. Chúng tôi chấp nhận điều này vì mục tiêu sản phẩm là hỗ trợ học tập nhanh chóng, giá cả phải chăng dưới mức trần chi phí trong 48 giờ; các mô hình cao hơn chỉ được dự phòng khi chất lượng yêu cầu.
